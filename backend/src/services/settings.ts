...
export default class SettingService {
  constructor(
    ...
    @Inject("establishmentModel")
    private establishmentModel: Models.EstablishmentModel,
    @Inject("logger") private logger,
    @EventDispatcher() private eventDispatcher: EventDispatcherInterface
  ) {}

  public async Save(
    establishmentInputDTO: IEstablishmentInputDTO,
    establishmentId: string
  ): Promise<{ establishment: IEstablishment }> {
    try {
      this.logger.silly("Creating user db record");
      const query = {},
        update = { ...establishmentInputDTO, establishmentId },
        options = { upsert: true, new: true, setDefaultsOnInsert: true };
      const establishmentRecord = await this.establishmentModel.findOneAndUpdate(
        query,
        update,
        options
      );
  ...
  public async GetGeneral(establishmentId: string) {
    try {
      const establishmentRecord = await this.establishmentModel.findOne({
        establishmentId,
      });
