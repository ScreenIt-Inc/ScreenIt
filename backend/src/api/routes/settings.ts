...
export default (app: Router) => {
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        await settingServiceInstance.Save(
          req.body as IEstablishmentInputDTO,
          req.currentUser.establishmentId
        );
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        const {
          establishment,
          userRecord,
        } = await settingServiceInstance.GetGeneral(
          req.currentUser.establishmentId
        );
        return res
          .status(201)
          .json({ generalSettings: establishment, permissions: userRecord });
      } catch (e) {
        logger.error("🔥 error: %o", e);
        return next(e);
      }
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        const settingServiceInstance = Container.get(SettingService);
        await settingServiceInstance.AddUser(
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        const settingServiceInstance = Container.get(SettingService);
        await settingServiceInstance.SaveUser(
          req.body as IUserInputDTO,
          req.currentUser.establishmentId
        );
        return res.status(201).json({ success: true });
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        const settingServiceInstance = Container.get(SettingService);
        await settingServiceInstance.DeleteUser(
          req.body.emails,
          req.currentUser.establishmentId
        );
        return res.status(201).json({ success: true });
